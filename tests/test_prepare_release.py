...
def test_get_current_version_requires_exactly_one_assignment() -> None:
    ...
    with pytest.raises(RuntimeError, match="Expected exactly one"):
        get_current_version(content, Path("app/__init__.py"))


def test_update_version_file() -> None:
    content = '"""Sandbox."""\n\n__version__ = "0.26.2"\n'

    new_content = update_version_file(content, "0.26.3", Path("app/__init__.py"))

    assert new_content == '"""Sandbox."""\n\n__version__ = "0.26.3"\n'
...
def test_update_version_file_requires_newer_version() -> None:
    ...
    with pytest.raises(RuntimeError, match="must be greater"):
        update_version_file(content, "0.26.2", Path("app/__init__.py"))
...
"""
    ...
    new_content = update_release_notes(
        content, "0.26.3", date(2026, 5, 28), Path("docs/en/docs/release-notes.md")
    )
...
def test_update_release_notes_requires_release_notes_header() -> None:
...
def test_update_release_notes_requires_latest_changes_header() -> None:
    content = "# Release Notes\n\n### Fixes\n"
...
"""
    ...
    body = get_release_notes_body(
        content, "0.26.3", Path("docs/en/docs/release-notes.md")
    )
